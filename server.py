...
'''
# SSLContext - This part is replaceable by:
...
] # Whatever is in what your `openssl ciphers | sed 's/:/\n/g'` spits out.
  # Might want to check https://wiki.mozilla.org/Security/Cipher_Suites
ssl_context.set_ciphers(':'.join(ALLOWED_CIPHERS))
# ==== Here is to be filled ===
# See https://docs.python.org/3/library/ssl.html#ssl.SSLContext.load_cert_chain
SSL_CERTIFICATE_PATH = 'cert.pem'
SSL_CERTIFICATE_PRIVATE_KEY = 'key.pem' # None or /path/to/privkey.pem
SSL_ENCRYPTED_KEY_PASSWORD = 'ejemplo' # or None

def run(addr, port):
    insecure_server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    insecure_server_socket.setblocking(0)
    insecure_server_socket.bind((addr, port))
    insecure_server_socket.listen()
    with ssl_context.wrap_socket(insecure_server_socket,
                                 server_side=True) as server_socket:
...
def init():
    ...
    global ssl_context
    ssl_context.load_cert_chain(SSL_CERTIFICATE_PATH,
                                SSL_CERTIFICATE_PRIVATE_KEY,
                                password=SSL_ENCRYPTED_KEY_PASSWORD)
